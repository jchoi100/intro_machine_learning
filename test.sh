...
for opt in "${options[@]}"; do
    for algo in "${algorithm[@]}"; do
        ...
        trainout=$(python ${1}classify.py --mode train --algorithm $algo --model-file ${opt}.${algo}.model --data ${opt}.train 2>&1)

        # capture stderr in a vara
        testout=$(python ${1}classify.py --mode test --model-file ${opt}.${algo}.model --data ${opt}.dev --predictions-file ${opt}.dev.predictions 2>&1)
